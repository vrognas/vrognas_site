...
description: "ATC J01 antibacterials for systemic use for modelers: the index is class-specific; resistance is a compartment."
date: "2026-02-20"
---
...
The traditional paradigm in severe infection is the interaction between the bug and the drug, and its central fallacy is that it misses the role of the host [@shorrAntibioticsCriticallyIll2012].
...
Decided by plotting log-kill against AUC, and fitting a nonlinear (sigmoid) regression.
Highest R^2 decides the index.

::: {.callout-warning}
R^2 is used by convention, even though it's not suitable for nonlinear regressions.
:::
...
For antibacterials, exposure is summarized as **unbound** concentration referenced to the **MIC**: *f*T~>MIC~, *f*AUC/MIC or *f*C~max~/MIC [@moutonStandardizationPharmacokineticPharmacodynamic2005].
...
Regrowth is usually modeled as a pre-existing less susceptible subpopulation that treatment selects for, though published models differ in how resistance arises [@jumbeApplicationMathematicalModel2003; @nielsenPharmacokineticPharmacodynamicModelingAntibacterial2013].

**The host sets the exposure.**
An index is only as good as the concentration achieved, and in the critically ill that is not the concentration the dose was chosen for.
Among critically ill patients given β-lactams, 42% had initial trough concentrations below the MIC; where measured creatinine clearance was ≥130 mL/min/1.73 m^2^ that figure was 82% [@udySubtherapeuticInitialLactam2012].
