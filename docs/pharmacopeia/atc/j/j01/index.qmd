...
After incubation, a well with growth is cloudy (turbid) or has a sediment at the bottom, and the MIC is the lowest concentration in a clear well (@fig-mic-plate).
...
The inoculum of a broth microdilution test is checked this way, by plating a sample from the growth control well.
For scale, the turbidity standard used to set up the inoculum (0.5 McFarland) matches 1 to 2 × 10^8^ CFU/mL [@wiegandAgarBrothDilution2008], so a well that starts at 5 × 10^5^ CFU/mL has to multiply 20- to 200-fold before its growth becomes visible.
...
Dose fractionation studies identify which index best predicts efficacy [@craigPharmacokineticPharmacodynamicParameters1998], and simulations then predict outcomes in diverse patient populations.
