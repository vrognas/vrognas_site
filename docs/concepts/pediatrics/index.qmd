...
description-meta: "Essential guide to pediatric pharmacometrics covering ontogeny, physiological differences between children and adults, allometric scaling, and model-informed drug development approaches in pediatric populations."
date: "2026-02-20"
keywords:
  - Pharmacometrics
...
::: {.callout-note title="MIDD approaches"}
...
```nmtran {#lst-v lst-cap="Allometric scaling for Volume"}
...
```nmtran {#lst-cl lst-cap="Allometric scaling for Clearance"}
...
```nmtran {#lst-cl-bedside lst-cap="Schwartz bedside formula [@schwartzNewEquationsEstimate2009]"}
CLR = 0.413 * HT / SCr ; mL/min/1.73 m2
```

But more commonly, the Rhodin formula is seen (@lst-cl-mat).

```nmtran {#lst-cl-mat lst-cap="Renal maturation [@rhodinHumanRenalFunction2009]. PMA from the input data."}
...
![](../images/pediatric-cov.png){fig-alt="Two scatter plots of predicted steady-state concentration, against body weight on the left and against age on the right, showing individual predictions, the simulated pediatric median and 90% interval, and the adult reference band."}

Predicted C~ss~ by **body weight (*left*)** and **age (*right*)** using the final pediatric population PK model (*red circles*: individual predictions).
The *blue line* is the median simulated pediatric C~ss~ and the *blue shaded area* encompasses 90% of the simulated pediatric patients.
The *horizontal gray band* encompasses 90% of the simulated adult patients receiving 100 mg BID.

:::

The figure is taken from the brivaracetam pediatric population pharmacokinetic analysis [@schoemakerBrivaracetamPopulationPharmacokinetics2017].
...
Thus, **response (E-R) and PD models from adults may <span class="red-underline">not apply to children</span>**.
