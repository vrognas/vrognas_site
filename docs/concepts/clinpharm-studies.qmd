...
![Drug Development Double Diamond.](images/learn-confirm-cycles){#fig-cycles}
...
* In combined SAD-MAD studies, subjects can participate in both parts. Add a PERIOD/PART covariate and calculate TIME/TAD/TAFD for each study part in those cases. 
* Dosing information can come in different forms (EX or EC domain):
  ...
  * Missed doses are sometimes not well documented; extra information can be included in the SUPPEX domain. 
...
The study can also be set up in a cross-over design, where both sequences of the conditions are observed for each subject (the first sequence is fasted-fed, and the second sequence is fed-fasted). 
