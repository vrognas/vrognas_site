...
| Generalized log-logistic^[If $\kappa = \rho$ then this reduces to the log-logistic.] [@khanGeneralizedLoglogisticProportional2016] | $\frac{\rho \lambda (\rho t)^{\lambda - 1}}{1 + (\kappa t)^\lambda}, (t; \rho; \kappa; \lambda) > 0$ | ✅ | ❌ |
...
<details>
  ...
  DADT(1) = SCALE * SHAPE * (SCALE * (TX - TP))**(SHAPE - 1) ; Integration of hazard
...
<details>
  ...
  SCALE = THETA(1) * EXP(ETA(1)) ; Sigma/Standard deviation
  LOCATION = THETA(2) ; Mu/Mean
  ...
  ;  PHI() is a builtin NONMEM function
  DADT(1) = (EXP(-(0.5 * Z**2)) / (SCALE * TX * SQRT(2 * PI))) / (1 - PHI(Z)) ; Integration of hazard.
  ...
  HAZNOW = (EXP(-(0.5 * ZX**2)) / (SCALE * TIMX * SQRT(2 * PI))) / (1 - PHI(ZX))
  
  IF (DV == 0) Y = SUR ; Censored event
  IF (DV == 1) Y = SUR * HAZNOW ; Event

  $THETA (0, 1, Inf) ; Scale
  $THETA (0, 2, Inf) ; Location
