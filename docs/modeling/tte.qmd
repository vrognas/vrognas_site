...
| :----------- | :--- | :--- | :---: | :---: |
| Exponential  | $\lambda$ | $\lambda = e^{\beta_0}$ | $\lambda = e^{-\alpha_0}$ |
| Weibull^[Pronounced "Veibull", with "v" as in "very"] | $\rho \gamma (\rho t)^{\gamma - 1}$ | ✅ | ✅ |
| Gompertz     | $\lambda e^{\gamma t}, 0 \le t, \lambda > 0$ | ✅ | ❌ |
| Log-logistic | $\frac{\rho \gamma(\rho t)^{\gamma - 1}}{1 + (\rho t)^\gamma}, 0 \le t, \gamma > 0$ | ❌ | ✅ |
| Generalized log-logistic^[If $\kappa = \rho$ then this reduces to the log-logistic.] [@khanGeneralizedLoglogisticProportional2016] | $\frac{\rho \gamma (\rho t)^{\gamma - 1}}{1 + (\kappa t)^\gamma}, (t; \rho; \kappa; \gamma) > 0$ | ✅ | ❌ |
| Log-normal^[The shape of the lognormal distribution is very similar to the log-logistic distribution and yields similar model results. @kleinbaumSurvivalAnalysisSelfLearning2012] | $\frac{(\sigma t \sqrt{2 \pi})^{-1} e^{-(\frac{1}{2}Z^2)}}{1 - \Phi(Z)}, Z = \frac{\ln{t} - \mu}{\sigma}$ | ❌ | ✅ |
...
The survivor function S(t) is calculated numerically, see @eq-surv.
:::
...
To see this linear relationship: start with the Weibull survival function $S(t) = e^{-\lambda t^p}$, take the log of $S(t)$, multiply by negative one, and take the log again.
For the Weibull distribution, the $\ln{-\ln{S(t)}}$ is a linear function of $\ln{t}$ with slope *p* and intercept $p \ln{\lambda}$.
