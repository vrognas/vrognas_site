...
<details>
  ...
  RATE    = THETA(1) * EXP(ETA(1))
  SHAPE_1 = THETA(2)
  SHAPE_2 = THETA(3) ; If equal to SHAPE_1, then reduces to log-logistic
  ...
  DADT(1) = (RATE * SHAPE_1 * ((RATE * TX)**(SHAPE_1 - 1))) / &
  (1 + (SHAPE_2 * TX)**SHAPE_1)
  ...
  HAZNOW = (RATE * SHAPE_1 * ((RATE * TIMX)**(SHAPE_1 - 1))) / &
  (1 + (SHAPE_2 * TIMX)**SHAPE_1)
  
  IF (DV == 0) Y = SUR ; Censored event
  IF (DV == 1) Y = SUR * HAZNOW ; Event

  $THETA (-Inf,  1, Inf) ; Rate
  $THETA (-Inf, -2, Inf) ; Shape 1
  $THETA (-Inf, -1, Inf) ; Shape 2, unique to the generalized form of log-logistic
