...
When finding the individual ETAs, NONMEM by default uses an optimization method called BFGS (Broyden, Fletcher, Goldfarb, and Shanno).
This method can be changed, but should for all practical purposes be left to its default.
The optimal ETA values are also called either Empirical Bayes Estimates (EBEs), Conditional Parametric ETAs (CPEs), or conditional mode estimates, referred to symbolically as $\hat{\eta}$ (pronounced "ETA hat").
:::
...
But for numerical integration problems `NSIG=2` is the largest one should insist on^[Credit: Robert Bauer].
...
How one should approach these options are different depending on if you are dealing with an analytical of numerical problem.
