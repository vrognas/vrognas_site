---
title: "NONMEM estimation methods"
...
The process of finding the individual ETAs is often called "Maximum a Posteriori (**MAP**) estimation"^[In the pharmacometric field, also called "mode a posteriori". Essentially the same, since the mode is the maximum of a unimodal distribution.].
...
When finding the individual ETAs, NONMEM by default uses an optimization method called **BFGS** (Broyden-Fletcher-Goldfarb-Shanno) [@fletcherPracticalMethodsOptimization2010].
...
NONMEM comes with a library of **ADVAN** routines.
...
| Routine | Control records | Description |
| :-----  | :---------- | :---------- |
...
| `ADVAN5`  | `$MODEL`| General Linear Model |
| `ADVAN6`  | `$MODEL`, `$DES`| General Nonlinear Model |
| `ADVAN7`  | `$MODEL`| General Linear Model with Real Eigenvalues |
| `ADVAN8`  | `$MODEL`, `$DES`| General Nonlinear Model with Stiff Differential Equations |
| `ADVAN9`  | `$MODEL`, `$DES`, `$AES`, `$AESINITIAL`| General Nonlinear Model with Equilibrium Compartments |
...
| `ADVAN13` | `$MODEL`, `$DES`| General Nonlinear Model using LSODA |
| `ADVAN14` | `$MODEL`, `$DES`| General Nonlinear Model using CVODA |
| `ADVAN15` | `$MODEL`, `$DES`| General Nonlinear Model with Equilibrium Compartments using IDA |
| `ADVAN16` | `$MODEL`, `$DES`| Delay Differential Equation Model using RADAR5 |
| `ADVAN17` | `$MODEL`, `$DES`, `$AES`, `$AESINITIAL`| Delay Differential Equation Model with Equilibrium Compartments using RADAR5 |
| `ADVAN18` | `$MODEL`, `$DES`| Delay Differential Equation Model using DDE_SOLVER |

: Advancement routines. {#tbl-advan tbl-colwidths="[20, 30, 50]"}
</details>

Then there are **TRANS** routines.
These routines are used to "translate" between parameterizations (hence "TRANS").

<details>
<summary>Show TRANS table</summary>
| Routine  | Description |
| :------- | :---------- |
...
: Translator routines. {#tbl-trans tbl-colwidths="[20, 80]"}
</details>
...
We differentiate problems by their computational nature, as this in some sense define the different options we want to tweak to optimize speed and accuracy of our estimation algorithm.
We define "numerical integration problems" as problems using `$DES`, and "analytical problems" those that do not.

### Fine-tuning tolerance

Tolerance options may be set at `$SUBROUTINES`, which be in effect throughout the entire problem^[An alternative method is to use the `$TOL` record, where options for each compartment may be entered.].
These options are used for the numerical `ADVAN` routines.
...
1. A **MAP estimation** (set by `MAPITER=1`) is performed to estimate the parameters of the proposal density (aka "sampling distribution" *q(x)*).
...
* `Fit.`: Average fitness.
This tells you how Normal the posterior is.
...
#### Full OMEGA blocks {#sec-em-robust-omega-block}
...
#### For uncertain initial estimates {#sec-em-robust-anneal}
...
The convergence test is defined by these four options:
...
::: {.callout-tip title="Allow time to converge"}
...
#### Paralell NONMEM {#sec-em-repr-paralell}
