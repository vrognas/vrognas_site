...
The process of finding the individual ETAs is often called "Mode a Posteriori (**MAP**) estimation".
...
It is also, to the authors best knowledge, a very similar method the the ones used to update population parameters in FO.
:::
...
$ESTIMATION METHOD=CONDITIONAL
            ...
            PRINT=1
```
1. `INTERACTION` should always be used when residual error is proportional, and does not impact the analysis when the residual error is additive. Thus, always use it.
...
6. `SIGLO <= TOL` and `SIGLO >= SIGL`. By default, `SIGLO=SIGL`
...
**If you can MU-model all THETAs**, then set `LDF=-1` in `$PK` to let NONMEM do the correction automatically.
...
Taking the **average OFV of these 5 iterations** will give a more accurate estimation of the final OFV[^JM].
