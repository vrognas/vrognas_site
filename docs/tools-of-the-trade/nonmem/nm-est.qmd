...
`TOL`: The relative significant digits precision to which differential equations are to be integrated, so the precision is 10^-`TOL`^

For numerical integration problems:

* Internal precision is based on user specified `TOL`, not `SIGL`
* Often `TOL` = 6 or less, not 10
* The largest `NSIG` one should insist on is 2.
...
#### AUTO option {#sec-impmap-options}
...
::: {.callout-note title="A note on sparse data"}
