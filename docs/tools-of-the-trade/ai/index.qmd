...
* Do not enable MCP's by default. Only enable them when you know you'll need them, then disable them. This is because they eat up your context window.
* Start with brainstorming/planning, then move on to implementation
  * Force (ultra)thinking.
  * Prompt: "Do not generate code until you are fully ready"
* Document the plan in a markdown document.
* Speaking of markdown documents, keep a `COMMON_MISTAKES.md` and reference it every time.
...
* Always write a *test* so that the AI can iterate (called *test-driven development*)
  * ~3 e2e tests per feature is enough
* "Stop if you find anything unexpected"

#### Tips 'n Tricks with agentic coding
